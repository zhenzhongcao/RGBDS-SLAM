...
public:
    ...
    // Constructor for RGB-DS cameras.
    Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const cv::Mat &imRGB, const cv::Mat &imS, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera,Frame* pPrevF = static_cast<Frame*>(NULL), const IMU::Calib &ImuCalib = IMU::Calib());
    ...
    // Backprojects a keypoint (if stereo/depth info available) into 3D world coordinates.
    bool UnprojectStereo(const int &i, Eigen::Vector3f &x3D, Eigen::Vector3f &colorRGB, Eigen::Vector3f &colorSemantic);
...
public:
    ...
    cv::Mat imgLeftRGB, imgAuxiliary, imgSemantic;
