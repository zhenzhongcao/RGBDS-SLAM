        ...
        for(int ikp=0; ikp<nmatches; ikp++)
        {
                                                                          ...
                                                                          (cosParallaxRays<0.9996 && mbInertial) || (cosParallaxRays<0.9998 && !mbInertial)))
            {
                ...
                if (mpCurrentKeyFrame->NLeft == -1)
                {
                    ...
                    colorRGB.y() = color[1];
                    colorRGB.z() = color[2];
                    const auto& semantic = mpCurrentKeyFrame->imgSemantic.at<cv::Vec3f>(v, u);
            ...
            else if(bStereo1 && cosParallaxStereo1<cosParallaxStereo2)
            {
                countStereoAttempt++;
                bPointStereo = true;
                goodProj = mpCurrentKeyFrame->UnprojectStereo(idx1, x3D, colorRGB, colorSemantic);
            }
            else if(bStereo2 && cosParallaxStereo2<cosParallaxStereo1)
            {
                countStereoAttempt++;
                bPointStereo = true;
                goodProj = pKF2->UnprojectStereo(idx2, x3D, colorRGB, colorSemantic);
            }
            else
            {
            ...
            pMP->SetColorSemantic(colorSemantic);
            if (bPointStereo)
                countStereo++;
