...
Sophus::SE3f System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const cv::Mat &semanticmap, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    cv::Mat imToFeed = preprocessImage(im);
    cv::Mat imDepthToFeed = depthmap.clone();
    cv::Mat imSemanticToFeed = semanticmap.clone();
    if(settings_ && settings_->needToResize()){
        cv::Mat resizedIm;
        cv::resize(im,resizedIm,settings_->newImSize());
        imToFeed = resizedIm;
        cv::resize(depthmap,imDepthToFeed,settings_->newImSize());
        cv::resize(semanticmap,imSemanticToFeed,settings_->newImSize());
    }
    ...
    Sophus::SE3f Tcw = mpTracker->GrabImageRGBD(imToFeed,imDepthToFeed,imSemanticToFeed,timestamp,filename);
