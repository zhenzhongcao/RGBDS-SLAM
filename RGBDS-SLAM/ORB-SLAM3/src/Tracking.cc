...
void Tracking::newParameterLoader(Settings *settings) {
    ...
    mImageScale = settings->getImageScale();
...
bool Tracking::ParseCamParamFile(cv::FileStorage &fSettings)
{
    ...
    {
        float fx, fy, cx, cy;
        mImageScale = fSettings["Camera.scale"];
...
Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const cv::Mat &imS, const double &timestamp, string filename)
{
    mImGray = imRGB;
    cv::Mat imDepth = imD;
    mImSemantic = imS.clone();
    ...
    if (mImSemantic.type() == CV_8UC3)
        mImSemantic.convertTo(mImSemantic, CV_32FC3, 1.0 / 255.0);
    else if (mImSemantic.type() == CV_16UC3)
        mImSemantic.convertTo(mImSemantic, CV_32FC3, 1.0 / 65535.0);
    else if (mImSemantic.type() == CV_16FC3 || mImSemantic.type() == CV_64FC3)
        mImSemantic.convertTo(mImSemantic, CV_32FC3, 1.0);

    if((fabs(mDepthMapFactor-1.0f)>1e-5) || imDepth.type()!=CV_32F)
        imDepth.convertTo(imDepth,CV_32F,mDepthMapFactor);

    if (mSensor == System::RGBD)
        mCurrentFrame = Frame(mImGray,imDepth,mImRGB,mImSemantic,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera);
    else if(mSensor == System::IMU_RGBD)
        mCurrentFrame = Frame(mImGray,imDepth,mImRGB,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,&mLastFrame,*mpImuCalib);
    ...
    if(mCurrentFrame.N>500)
    {
        ...
        if(!mpCamera2){
            for(int i=0; i<mCurrentFrame.N;i++)
            {
                ...
                if(z>0)
                {
                    Eigen::Vector3f x3D, colorRGB, colorSemantic;
                    mCurrentFrame.UnprojectStereo(i, x3D, colorRGB, colorSemantic);
                    MapPoint* pNewMP = new MapPoint(x3D, colorRGB, pKFini, mpAtlas->GetCurrentMap());
                    pNewMP->SetColorSemantic(colorSemantic);
