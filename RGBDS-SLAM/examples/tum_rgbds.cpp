...
int main(int argc, char **argv)
{
    ...
    std::shared_ptr<ORB_SLAM3::System> pSLAM =
        std::make_shared<ORB_SLAM3::System>(
            argv[1], argv[2], ORB_SLAM3::System::RGBD, true);
    ...
    for (int ni = 0; ni < nImages; ni++)
    {
        ...
        imS = cv::imread(std::string(argv[4]) + "/semantic/" + std::to_string(vTimestamps[ni]) + ".png", cv::IMREAD_UNCHANGED);
        ...
        if (imS.empty())
        {
            std::cerr << std::endl << "Failed to load semantic image at: "
                      << std::string(argv[4]) << "/" << vstrImageFilenamesRGB[ni] << std::endl;
            return 1;
        }
        ...
        if (imageScale != 1.f)
        {
            ...
            cv::resize(imS, imS, cv::Size(width, height));
        }

        std::chrono::steady_clock::time_point t1 = std::chrono::steady_clock::now();

        // Pass the image to the SLAM system
        pSLAM->TrackRGBD(imRGB, imD, imS, tframe, std::vector<ORB_SLAM3::IMU::Point>(), vstrImageFilenamesRGB[ni]);
