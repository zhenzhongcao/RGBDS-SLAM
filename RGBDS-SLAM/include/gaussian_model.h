...
#define GAUSSIAN_MODEL_TENSORS_TO_VEC                        \
    ...
    this->Tensor_vec_semantic_feature_dc_ = {this->semantic_features_dc_};     \
    this->Tensor_vec_semantic_feature_rest_ = {this->semantic_features_rest_}; \
...
#define GAUSSIAN_MODEL_INIT_TENSORS(device_type)                                             \
    ...
    this->semantic_features_dc_ = torch::empty(0, torch::TensorOptions().device(device_type));        \
    this->semantic_features_rest_ = torch::empty(0, torch::TensorOptions().device(device_type));      \
...
public:
    ...
    torch::Tensor getSemanticFeatures();
    ...
    void increasePcd(std::vector<float> points, std::vector<float> colors, std::vector<float> semantics, const int iteration);
    void increasePcd(torch::Tensor& new_point_cloud, torch::Tensor& new_colors, const int iteration);
    void increasePcd(torch::Tensor& new_point_cloud, torch::Tensor& new_colors, torch::Tensor& new_semantics, const int iteration);
    ...
    void setSemanticFeatureLearningRate(float feature_lr);
...
public:
    ...
    torch::Tensor semantic_features_dc_;
    torch::Tensor semantic_features_rest_;
    ...
    std::vector<torch::Tensor> Tensor_vec_xyz_,
                               ...
                               Tensor_vec_semantic_feature_dc_,
                               Tensor_vec_semantic_feature_rest_,
                               Tensor_vec_opacity_,
                               Tensor_vec_scaling_ ,
    ...
    torch::Tensor sparse_points_semantic_;
