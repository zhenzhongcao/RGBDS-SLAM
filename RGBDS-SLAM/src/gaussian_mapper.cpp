...
void GaussianMapper::readConfigFromFile(std::filesystem::path cfg_path)
{
    ...
    max_num_iterations=settings_file["Mapper.max_num_iterations"].operator int();
    ...
    opt_use_semantic_ =
        (settings_file["Optimization.useSemantic"].operator int()) != 0;
...
void GaussianMapper::run()
{
        ...
        if (hasMetInitialMappingConditions()) {
            ...
            std::vector<ORB_SLAM3::MapPoint*> vpMPs;
            {
                ...
                for (const auto& pMP : vpMPs){
                    ...
                    // std::cout<<"point3D.color_(0) = "<<point3D.color_(0)<<
                    // "point3D.color_(1) = "<<point3D.color_(1)<<
                    // "point3D.color_(2) = "<<point3D.color_(2)<<std::endl;
                    auto semantic = pMP->GetColorSemantic();
                    point3D.semantic_color_(0) = semantic(0);
                    point3D.semantic_color_(1) = semantic(1);
                    point3D.semantic_color_(2) = semantic(2);
                    // std::cout<<"point3D.semantic_color_(0) = "<<point3D.semantic_color_(0)<<
                    // "point3D.semantic_color_(1) = "<<point3D.semantic_color_(1)<<
                    // "point3D.semantic_color_(2) = "<<point3D.semantic_color_(2)<<std::endl;
                ...
                for (const auto& pKF : vpKFs){
                    ...
                    cv::Mat imgRGB_undistorted, imgAux_undistorted, imgSem_undistorted;
                    try {
                        // Camera
                        ...
                        // Semantic Image
                        cv::Mat imgSem = pKF->imgSemantic;
                        if(!imgSem.empty())
                        {
                            if (this->sensor_type_ == RGBD)
                            camera.undistortImage(imgSem, imgSem_undistorted);
                            else
                                imgSem_undistorted = imgSem;
                        }
                        ...
                        new_kf->semantic_image_ =
                            tensor_utils::cvMat2TorchTensor_Float32(imgSem_undistorted, device_type_);
                    ...
                    new_kf->img_semantic_undist_ = imgSem_undistorted;
                }
            }
            ...
            for (auto& kfit : scene_->keyframes()) {
                ...
                if (device_type_ == torch::kCUDA) {
                    ...
                    img_gpu.upload(pkf->img_semantic_undist_);
                    pkf->gaus_pyramid_semantic_image_.resize(num_gaus_pyramid_sub_levels_);
                    for (int l = 0; l < num_gaus_pyramid_sub_levels_; ++l) {
                        cv::cuda::GpuMat img_resized;
                        cv::cuda::resize(img_gpu, img_resized,
                                        cv::Size(pkf->gaus_pyramid_width_[l], pkf->gaus_pyramid_height_[l]));
                        pkf->gaus_pyramid_semantic_image_[l] =
                            tensor_utils::cvGpuMat2TorchTensor_Float32(img_resized);
                    }
                }
                else {
                    ...
                    for (int l = 0; l < num_gaus_pyramid_sub_levels_; ++l) {
                        cv::Mat img_resized;
                        cv::resize(pkf->img_semantic_undist_, img_resized,
                                cv::Size(pkf->gaus_pyramid_width_[l], pkf->gaus_pyramid_height_[l]));
                        pkf->gaus_pyramid_semantic_image_[l] =
                            tensor_utils::cvMat2TorchTensor_Float32(img_resized, device_type_);
                    }
    ...
    while (getIteration() <  max_num_iterations || getIteration() - SLAM_stop_iter <= n_delay_iters || getIteration() % densify_interval <= n_delay_iters || isKeepingTraining()) {
        trainForOneIteration();
...
void GaussianMapper::trainForOneIteration()
{
    ...
    torch::Tensor gt_image, mask, gt_depth_image, gt_semantic_image;
    ...
    if (training_level == num_gaus_pyramid_sub_levels_) {
        ...
        gt_semantic_image = viewpoint_cam->semantic_image_.cuda();
    }
    else {
        ...
        gt_semantic_image = viewpoint_cam->gaus_pyramid_semantic_image_[training_level].cuda();
    }
    ...
    auto rendered_semantic_image = std::get<5>(render_pkg);

    // Get rid of black edges caused by undistortion
    torch::Tensor masked_image = rendered_image * mask;
    torch::Tensor masked_semantic_image = rendered_semantic_image * mask;
    ...
    auto Ll1_semantic = loss_utils::l1_loss(masked_semantic_image, gt_semantic_image);
    ...
    loss = (1.0 - lambda_dssim) * Ll1
                + lambda_dssim * (1.0 - loss_utils::ssim(masked_image, gt_image, device_type_));
    if(opt_use_depth_ && opt_use_semantic_){
        loss += (1.0 - lambda_dssim) * Ll1_semantic
                + lambda_dssim * (1.0 - loss_utils::ssim(masked_semantic_image, gt_semantic_image, device_type_))
                + Ll1_depth;
        // std::cout<<"Ll1 = "<<Ll1.item()<<"  Ll1_depth = "<<Ll1_depth.item()<<"   Ll1_semantic = "<<Ll1_semantic.item()<<std::endl;
    }else if(opt_use_depth_){
        loss += Ll1_depth;
    }else if(opt_use_semantic_){
        loss += (1.0 - lambda_dssim) * Ll1_semantic
                + lambda_dssim * (1.0 - loss_utils::ssim(masked_semantic_image, gt_semantic_image, device_type_));
    }
    loss.backward();
...
void GaussianMapper::combineMappingOperations()
{
    ...
    while (pSLAM_->getAtlas()->hasMappingOperation()) {
        ...
        case ORB_SLAM3::MappingOperation::OprType::LocalMappingBA:
        {
            ...
            auto& semantics = std::get<2>(associated_points);

            // Add new points to the model
            if (initial_mapped_ && points.size() >= 30) {
                torch::NoGradGuard no_grad;
                std::unique_lock<std::mutex> lock_render(mutex_render_);
                gaussians_->increasePcd(points, colors, semantics, getIteration());
            }
        }
        break;
                ...
                std::string> &kf)
{
    ...
    pkf->img_semantic_undist_ = imgSem_undistorted;
...
// savePly(result_dir_ / (std::to_string(getIteration()) + "_" + std::to_string(pkf->fid_) + "_1_after_inactive_geo_densify"));
    ...
    {
// savePly(result_dir_ / (std::to_string(getIteration()) + "_" + std::to_string(pkf->fid_) + "_0_before_inactive_geo_densify"));
        cv::cuda::GpuMat img_rgb_gpu, img_depth_gpu, img_semantic_gpu;
        img_rgb_gpu.upload(pkf->img_undist_);
        img_depth_gpu.upload(pkf->img_auxiliary_undist_);
        img_semantic_gpu.upload(pkf->img_semantic_undist_);
        ...
        torch::Tensor semantic = tensor_utils::cvGpuMat2TorchTensor_Float32(img_semantic_gpu);
        semantic = semantic.permute({1, 2, 0}).flatten(0, 1).contiguous();
        ...
        torch::Tensor semantics_valid = semantic.index({point_valid_flags});
        ...
        if (depth_cached_ == 0) {
            depth_cache_points_ = points3D_valid;
            depth_cache_colors_ = colors_valid;
            depth_cache_semantics_ = semantics_valid;
        }
        else {
            depth_cache_points_ = torch::cat({depth_cache_points_, points3D_valid}, /*dim=*/0);
            depth_cache_colors_ = torch::cat({depth_cache_colors_, colors_valid}, /*dim=*/0);
            depth_cache_semantics_ = torch::cat({depth_cache_semantics_, semantics_valid}, /*dim=*/0);
        }
// savePly(result_dir_ / (std::to_string(getIteration()) + "_" + std::to_string(pkf->fid_) + "_1_after_inactive_geo_densify"));
    }
    break;
    ...
    if (depth_cached_ >= max_depth_cached_) {
        ...
        // Add new points to the model
        std::unique_lock<std::mutex> lock_render(mutex_render_);
        gaussians_->increasePcd(depth_cache_points_, depth_cache_colors_, depth_cache_semantics_, getIteration());
    }
...
void GaussianMapper::recordKeyframeRenderedRGBDepthSemantic(
        torch::Tensor &rendered,
        torch::Tensor &ground_truth,
        torch::Tensor &rendered_depth,
        torch::Tensor &ground_truth_depth,
        torch::Tensor &rendered_semantic,
        torch::Tensor &ground_truth_semantic,
        unsigned long kfid,
        std::filesystem::path result_img_dir,
        std::filesystem::path result_gt_dir,
        std::filesystem::path result_depth_img_dir,
        std::filesystem::path result_depth_gt_dir,
        std::filesystem::path result_semantic_img_dir,
        std::filesystem::path result_semantic_gt_dir,
        std::filesystem::path result_loss_dir,
        std::string name_suffix)
{
    if (record_rendered_image_) {
    ...
    if (record_rendered_image_) {
        auto image_cv = tensor_utils::torchTensor2CvMat_Float32(rendered_semantic);
        cv::cvtColor(image_cv, image_cv, CV_RGB2BGR);
        image_cv.convertTo(image_cv, CV_8UC3, 255.0f);
        cv::imwrite(result_semantic_img_dir / (std::to_string(getIteration()) + "_" + std::to_string(kfid) + name_suffix + ".jpg"), image_cv);
    }

    if (record_ground_truth_image_) {
        auto gt_image_cv = tensor_utils::torchTensor2CvMat_Float32(ground_truth_semantic);
        cv::cvtColor(gt_image_cv, gt_image_cv, CV_RGB2BGR);
        gt_image_cv.convertTo(gt_image_cv, CV_8UC3, 255.0f);
        cv::imwrite(result_semantic_gt_dir / (std::to_string(getIteration()) + "_" + std::to_string(kfid) + name_suffix + "_gt.jpg"), gt_image_cv);
    }
    ...
    const bool main_vision)
{
    ...
    std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor, torch::Tensor> render_pkg;
    {
        std::unique_lock<std::mutex> lock_render(mutex_render_);
        // Render
...
cv::Mat GaussianMapper::renderSemanticFromPose(
    const Sophus::SE3f &Tcw,
    const int width,
    const int height,
    const bool main_vision)
{
    ...
    torch::Tensor masked_image;
    if (main_vision)
        masked_image = std::get<5>(render_pkg) * viewer_main_undistort_mask_[pkf->camera_id_];
    else
        masked_image = std::get<5>(render_pkg) * viewer_sub_undistort_mask_[pkf->camera_id_];
    return tensor_utils::torchTensor2CvMat_Float32(masked_image);
}
...
void GaussianMapper::renderAndRecordKeyframeRGBDepthSemantic(
    std::shared_ptr<GaussianKeyframe> pkf,
    float &dssim,
    float &psnr,
    float &psnr_gs,
    ...
    std::string name_suffix)
{
    ...
    auto rendered_semantic_image = std::get<5>(render_pkg);
    torch::Tensor masked_image = rendered_image * undistort_mask_[pkf->camera_id_];
    torch::Tensor masked_semantic_image = rendered_semantic_image * undistort_mask_[pkf->camera_id_];
    torch::cuda::synchronize();
    auto end_timing = std::chrono::steady_clock::now();
    auto render_time_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(end_timing - start_timing).count();
    render_time = 1e-6 * render_time_ns;
    auto gt_image = pkf->original_image_;
    auto gt_depth_image = pkf->depth_image_;
    auto gt_semantic_image = pkf->semantic_image_;
    ...
    recordKeyframeRenderedRGBDepthSemantic(masked_image, gt_image, rendered_depth_image, gt_depth_image, rendered_semantic_image, gt_semantic_image, pkf->fid_, result_img_dir, result_gt_dir, result_depth_img_dir, result_depth_gt_dir, result_semantic_img_dir, result_semantic_gt_dir, result_loss_dir, name_suffix);    
}
    ...
    std::string name_suffix)
{
    ...
    std::filesystem::path semantic_image_dir = result_dir / "semantic_image";
    if (record_rendered_image_)
        CHECK_DIRECTORY_AND_CREATE_IF_NOT_EXISTS(semantic_image_dir);

    std::filesystem::path semantic_image_gt_dir = result_dir / "semantic_image_gt";
    if (record_ground_truth_image_)
        CHECK_DIRECTORY_AND_CREATE_IF_NOT_EXISTS(semantic_image_gt_dir);
    ...
    for (std::size_t i = 0; i < nkfs; ++i) {
        renderAndRecordKeyframeRGBDepthSemantic((*kfit).second, dssim, psnr, psnr_gs, l1, render_time, image_dir, image_gt_dir, depth_image_dir, depth_image_gt_dir, semantic_image_dir, semantic_image_gt_dir, image_loss_dir);
        avg_dssim += dssim;
        avg_psnr += psnr;
        avg_psnr_gs += psnr_gs;
        avg_l1 += l1;
...
void GaussianMapper::saveSemanticPly(std::filesystem::path result_dir)
{
    CHECK_DIRECTORY_AND_CREATE_IF_NOT_EXISTS(result_dir)
    keyframesToJson(result_dir);
    saveModelParams(result_dir);
