...
{
    return torch::cat({this->semantic_features_dc_.clone(), this->semantic_features_rest_.clone()}, /*dim=*/1);
}
    ...
    const float spatial_lr_scale)
{
    ...
    torch::Tensor color = torch::zeros(
        {num_points, 3},
        torch::TensorOptions().dtype(torch::kFloat).device(device_type_));
    torch::Tensor semantic_color = torch::zeros(
        {num_points, 3},
        torch::TensorOptions().dtype(torch::kFloat).device(device_type_));
    ...
    for (int point_idx = 0; point_idx < num_points; ++point_idx) {
        ...
        semantic_color.index({point_idx, 0}) = point.semantic_color_(0);
        semantic_color.index({point_idx, 1}) = point.semantic_color_(1);
        semantic_color.index({point_idx, 2}) = point.semantic_color_(2);
        ++pcd_it;
    }

    torch::Tensor fused_color = sh_utils::RGB2SH(color);
    torch::Tensor fused_semantic_color = sh_utils::RGB2SH(semantic_color);
    ...
    torch::Tensor semantic_features = torch::zeros(
        {fused_semantic_color.size(0), 3, temp * temp},
        torch::TensorOptions().dtype(torch::kFloat).device(device_type_));
    features.index(
    ...
    semantic_features.index(
        {torch::indexing::Slice(),
         torch::indexing::Slice(0, 3),
         0}) = fused_semantic_color;
    semantic_features.index(
        {torch::indexing::Slice(),
         torch::indexing::Slice(3, semantic_features.size(1)),
         torch::indexing::Slice(1, semantic_features.size(2))}) = 0.0f;
    ...
    this->features_rest_ = features.index({torch::indexing::Slice(),
                               ...
                               .contiguous()
                               .requires_grad_();
    this->semantic_features_dc_ = semantic_features.index({torch::indexing::Slice(),
                                         torch::indexing::Slice(),
                                         torch::indexing::Slice(0, 1)})
                             .transpose(1, 2)
                             .contiguous()
                             .requires_grad_();
    this->semantic_features_rest_ = semantic_features.index({torch::indexing::Slice(),
                                           torch::indexing::Slice(),
                                           torch::indexing::Slice(1, semantic_features.size(2))})
                               .transpose(1, 2)
                               .contiguous()
                               .requires_grad_();
...
void GaussianModel::increasePcd(std::vector<float> points, std::vector<float> colors, std::vector<float> semantics, const int iteration)
{
// auto time1 = std::chrono::steady_clock::now();
    assert(points.size() == colors.size());
    assert(points.size() % 3 == 0);
    ...
    torch::Tensor new_colors = torch::from_blob(
        ...
        torch::TensorOptions().dtype(torch::kFloat32)).to(device_type_);
        // torch::zeros({num_new_points, 3}, xyz_.options());
    torch::Tensor new_semantics = torch::from_blob(
        semantics.data(), {num_new_points, 3},
        torch::TensorOptions().dtype(torch::kFloat32)).to(device_type_);
        // torch::zeros({num_new_points, 3}, xyz_.options());
    ...
    if (sparse_points_xyz_.size(0) == 0) {
        sparse_points_xyz_ = new_point_cloud;
        sparse_points_color_ = new_colors;
        sparse_points_semantic_= new_semantics;
    }
    else {
        sparse_points_xyz_ = torch::cat({sparse_points_xyz_, new_point_cloud}, /*dim=*/0);
        sparse_points_color_ = torch::cat({sparse_points_color_, new_colors}, /*dim=*/0);
        sparse_points_semantic_ = torch::cat({sparse_points_semantic_, new_semantics}, /*dim=*/0);
    }
    ...
    torch::Tensor new_fused_semantics = sh_utils::RGB2SH(new_semantics);
    torch::Tensor semantic_features = torch::zeros(
        {new_fused_semantics.size(0), 3, temp * temp},
        torch::TensorOptions().dtype(torch::kFloat).device(device_type_));
    semantic_features.index(
        {torch::indexing::Slice(),
         torch::indexing::Slice(0, 3),
         0}) = new_fused_semantics;
    semantic_features.index(
        {torch::indexing::Slice(),
         torch::indexing::Slice(3, semantic_features.size(1)),
         torch::indexing::Slice(1, semantic_features.size(2))}) = 0.0f;
    ...
    auto new_features_rest = features.index({torch::indexing::Slice(),
                                          ...
                                          .transpose(1, 2)
                                          .contiguous();
    auto new_semantic_features_dc = semantic_features.index({torch::indexing::Slice(),
                                                    torch::indexing::Slice(),
                                                    torch::indexing::Slice(0, 1)})
                                        .transpose(1, 2)
                                        .contiguous();
    auto new_semantic_features_rest = semantic_features.index({torch::indexing::Slice(),
                                                      torch::indexing::Slice(),
                                                      torch::indexing::Slice(1, semantic_features.size(2))})
                                          .transpose(1, 2)
                                          .contiguous();
...
// std::cout << "increasePcd(umap) preparation time: " << time << " ms" <<std::endl;
    ...
    densificationPostfix(
        ...
        new_features_dc,
        new_features_rest,
        new_semantic_features_dc,
        new_semantic_features_rest,
        new_opacities,
        new_scaling,
        new_rotation,
        new_exist_since_iter
...
void GaussianModel::increasePcd(torch::Tensor& new_point_cloud, torch::Tensor& new_colors, torch::Tensor& new_semantics, const int iteration)
{
// auto time1 = std::chrono::steady_clock::now();
    auto num_new_points = new_point_cloud.size(0);
    if (num_new_points == 0)
    ...
    torch::Tensor& new_scaling)
{
    ...
    torch::Tensor optimizable_scaling = this->replaceTensorToOptimizer(new_scaling, 6);
...
// scales = scales.unsqueeze(scales_ndimension).repeat({1, 3});
    ...
    // param_groups[3] = opacity_5
    // param_groups[4] = scaling_6
    // param_groups[5] = rotation_7
    // ==================================
    ...
    torch::Tensor optimizable_rots = this->replaceTensorToOptimizer(rots, 7);
...
void GaussianModel::trainingSetup(const GaussianOptimizationParams& training_args)
{
    ...
    optimizer_->add_param_group(Tensor_vec_semantic_feature_dc_);
    optimizer_->param_groups()[3].options().set_lr(training_args.feature_lr_);

    optimizer_->add_param_group(Tensor_vec_semantic_feature_rest_);
    optimizer_->param_groups()[4].options().set_lr(training_args.feature_lr_ / 20.0);

    optimizer_->add_param_group(Tensor_vec_opacity_);
    optimizer_->param_groups()[5].options().set_lr(training_args.opacity_lr_);

    optimizer_->add_param_group(Tensor_vec_scaling_);
    optimizer_->param_groups()[6].options().set_lr(training_args.scaling_lr_);

    optimizer_->add_param_group(Tensor_vec_rotation_);
    optimizer_->param_groups()[7].options().set_lr(training_args.rotation_lr_);
...
void GaussianModel::setSemanticFeatureLearningRate(float feature_lr)
{
    optimizer_->param_groups()[3].options().set_lr(feature_lr);
    optimizer_->param_groups()[4].options().set_lr(feature_lr / 20.0);
}
void GaussianModel::setOpacityLearningRate(float opacity_lr)
{
    optimizer_->param_groups()[5].options().set_lr(opacity_lr);
}
void GaussianModel::setScalingLearningRate(float scaling_lr)
{
    optimizer_->param_groups()[6].options().set_lr(scaling_lr);
}
void GaussianModel::setRotationLearningRate(float rot_lr)
{
    optimizer_->param_groups()[7].options().set_lr(rot_lr);
}
...
void GaussianModel::resetOpacity()
{
    ...
    torch::Tensor optimizable_tensors = this->replaceTensorToOptimizer(opacities_new, 5); // "opacity"
...
void GaussianModel::prunePoints(torch::Tensor& mask)
{
    ...
    // _prune_optimizer
    std::vector<torch::Tensor> optimizable_tensors(8);
    ...
    this->semantic_features_dc_ = optimizable_tensors[3];
    this->semantic_features_rest_ = optimizable_tensors[4];
    this->opacity_ = optimizable_tensors[5];
    this->scaling_ = optimizable_tensors[6];
    this->rotation_ = optimizable_tensors[7];
...
void GaussianModel::densificationPostfix(
    ...
    torch::Tensor& new_features_dc,
    torch::Tensor& new_features_rest,
    torch::Tensor& new_semantic_features_dc,
    torch::Tensor& new_semantic_features_rest,
    torch::Tensor& new_opacities,
    torch::Tensor& new_scaling,
    torch::Tensor& new_rotation,
    torch::Tensor& new_exist_since_iter)
    ...
    int N)
{
    ...
    auto new_semantic_features_dc = this->semantic_features_dc_.index({selected_pts_mask}).repeat({N, 1, 1});
    auto new_semantic_features_rest = this->semantic_features_rest_.index({selected_pts_mask}).repeat({N, 1, 1});
    ...
    float scene_extent)
{
    ...
    auto new_semantic_features_dc = this->semantic_features_dc_.index({selected_pts_mask});
    auto new_semantic_features_rest = this->semantic_features_rest_.index({selected_pts_mask});
