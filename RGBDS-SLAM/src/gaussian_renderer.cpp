    ...
    bool use_override_color)
{
    ...
    torch::Tensor shs,
                  colors_precomp, semantic_colors_precomp;
    if (use_override_color) {
        ...
        if (pipe.convert_SHs_) {
            ...
            shs_view = pc->getSemanticFeatures().transpose(1, 2).view({-1, 3, max_sh_degree * max_sh_degree});
            dir_pp = (pc->getXYZ() - viewpoint_camera->camera_center_.repeat({pc->getSemanticFeatures().size(0), 1}));
            dir_pp_normalized = dir_pp / torch::frobenius_norm(dir_pp, /*dim=*/{1}, /*keepdim=*/true);
            sh2rgb = sh_utils::eval_sh(pc->active_sh_degree_, shs_view, dir_pp_normalized);
            semantic_colors_precomp = torch::clamp_min(sh2rgb + 0.5, 0.0);

            // std::cout<<"colors_precomp = "<<colors_precomp.item()<<std::endl;
            // std::cout<<"semantic_colors_precomp = "<<semantic_colors_precomp.item()<<std::endl;
    ...
    auto rendered_semantic = std::get<3>(rasterizer_result);
