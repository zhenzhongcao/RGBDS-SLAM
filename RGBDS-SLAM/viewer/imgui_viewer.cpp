...
void ImGuiViewer::run()
{
    ...
    GLuint SLAM_img_texture, rendered_img_texture, rendered_depth_img_texture, rendered_semantic_img_texture, main_img_texture;
    ...
    glGenTextures(1, &rendered_semantic_img_texture);
    glBindTexture(GL_TEXTURE_2D, rendered_semantic_img_texture);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    ...
    while(!isStopped() && !glfwWindowShouldClose(window))
    {
        ...
        if (pSLAM_)
        {
            ...
            ImGui::SetNextWindowPos(ImVec2(SLAM_img_to_show.cols + 12, 0), ImGuiCond_Once);
            ...
            cv::Mat rendered_semantic_img = pGausMapper_->renderSemanticFromPose(
                Tcw, rendered_image_width_, rendered_image_height_, false);
            cv::Mat rendered_semantic_img_to_show = cv::Mat(rendered_image_height_, padded_sub_image_width_, CV_32FC3, cv::Vec3f(0.0f, 0.0f, 0.0f));
            rendered_semantic_img.copyTo(rendered_semantic_img_to_show(image_rect_sub));
            // Upload rendered frame
            glBindTexture(GL_TEXTURE_2D, rendered_semantic_img_texture);
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, rendered_semantic_img_to_show.cols, rendered_semantic_img_to_show.rows,
                        0, GL_RGB, GL_FLOAT, (float*)rendered_semantic_img_to_show.data);
            // Create an ImGui window to show the rendered frame
            ImGui::SetNextWindowPos(ImVec2(SLAM_img_to_show.cols + 12, SLAM_img_to_show.rows  + 40), ImGuiCond_Once);
            ImGui::SetNextWindowSize(ImVec2(rendered_image_width_ + 12, rendered_semantic_img_to_show.rows + 40), ImGuiCond_Once);
            {
                ImGui::Begin("Current Rendered Semantic Frame");
                ImGui::Image((void *)(intptr_t)rendered_semantic_img_texture,
                            ImVec2(rendered_semantic_img_to_show.cols, rendered_semantic_img_to_show.rows));
                ImGui::End();
            }
        ...
        // Draw main window semnatic image
        if (show_main_semantic_rendered_)
        {
            auto drawlist = ImGui::GetBackgroundDrawList();
            if (pSLAM_ && tracking_vision_)
            {
                drawlist->AddImage((void *)(intptr_t)rendered_semantic_img_texture, ImVec2(0, 0),
                                   ImVec2(glfw_window_width_, glfw_window_height_));
            }
            else
            {
                cv::Mat main_img = pGausMapper_->renderSemanticFromPose(
                    Tcw_main_, rendered_image_width_main_, rendered_image_height_main_, true);
                cv::Mat main_img_to_show = cv::Mat(rendered_image_height_main_, padded_main_image_width_, CV_32FC3, cv::Vec3f(0.0f, 0.0f, 0.0f));
                main_img.copyTo(main_img_to_show(image_rect_main));
                glBindTexture(GL_TEXTURE_2D, main_img_texture);
        ...
        ImGui::SetNextWindowSize(ImVec2(panel_width_, display_panel_height_), ImGuiCond_Once);
        {
            ...
            ImGui::Checkbox("Show main window semantic rendered", &show_main_semantic_rendered_);
